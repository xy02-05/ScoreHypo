...
from ..bbox import _box_to_center_scale, _center_scale_to_box,get_bbox,process_bbox
...
class SimpleTransform3DSMPL(object):
    ...
    def __call__(self, src, label):
        if self.two_d:
            ...
            bbox = process_bbox(bbox,aspect_ratio=self._aspect_ratio)
            xmin, ymin, w, h = bbox
            center, scale = _box_to_center_scale(
            ...
            # crop the img and joints to bbox
            inp_h, inp_w = input_size
            ...
            #target_uvd_29, target_weight_29 = self.generate_heatmap(joints[:,:,0].copy(), joints[:,:,1].copy() )
            #target, target_weight = self._integral_target_generator(joints, self.num_joints, inp_h, inp_w)
            target =  joints[:,:,0].copy()
            target_weight = joints[:,:,1].copy() 
        ...
        if self.two_d:
            ...
            output = {
                ...
                'joints_uvd_29':torch.from_numpy(joints_29_uvd[:,:,0]).float(),
                'joints_2d': torch.from_numpy(joints_2d_uvd[:,:,0]).float(),
                'joints_vis_29':torch.from_numpy(joints_29_uvd[:,:,1]).float(),
                'joints_xyz_24':torch.from_numpy(joints_24_xyz[:24,:]).float(),
                'joints_vis_24':torch.from_numpy(joints_vis_29[:24, :]).float(),
                'joints_xyz_17':torch.from_numpy(joints_17_xyz).float(),
                'joints_vis_17':torch.from_numpy(joints_vis_17).float(),
