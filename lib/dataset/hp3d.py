...
from utils.pose_utils import (cam2pixel_matrix, pixel2cam_matrix,reconstruction_error)
...
class HP3D(data.Dataset):
    ...
    def __init__(self,
        ...
        self._ann_file = os.path.join(
            root,'annotations', f'annotation_mpi_inf_3dhp_{ann_file}.json')
    ...
    def _load_jsons(self):
        ...
        for ann_image, ann_annotations in zip(database['images'], database['annotations']):
            ...
            abs_path = os.path.join(self._root,'images', 'mpi_inf_3dhp_{}_set'.format('train' if self._train else 'test'), ann['file_name'])
    ...
    def missing_joint(self):
        return ((6,7,8),(3,4,5),(12,13,14),(9,10,11))
